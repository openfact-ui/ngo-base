...
mkdir -p dist && docker run --detach=true --name=ngo-base-builder -e "OPENFACT_SYNC_API_URL=http://api.openshift.io/api/" -e JENKINS_URL -e GIT_BRANCH -e "CI=true" -e GH_TOKEN -e NPM_TOKEN -t -v $(pwd)/dist:/dist:Z ngo-base-builder
...
docker exec ngo-base-builder git checkout master
# Try to fix up the git repo so that npm publish can build the gitHead ref in to package.json
docker exec ngo-base-builder ./fix-git-repo.sh

# Build almigty-ui
docker exec ngo-base-builder npm install

## Build prod
docker exec ngo-base-builder npm run build
...
if [ $? -eq 0 ]; then
  ...
  set +e
  # Publish to npm
  docker exec ngo-base-builder npm run semantic-release
  if [ $? -eq 0 ]; then
